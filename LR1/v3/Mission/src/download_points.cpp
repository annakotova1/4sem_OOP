...
{
    int rc = fscanf(file, "%c %lf %lf %lf\n", &p.name, &p.coordinates.x, &p.coordinates.y, &p.coordinates.z);
    if (rc != 4)
    {
        fscanf(file, "\n");
        return END_READ;
...
int check_point(const shape_point &point, list_points_node *head)
{
    for (list_points_node *cur = head; cur; cur = cur->next)
    {
        ...
        if (cur->data.coordinates.x == point.coordinates.x && cur->data.coordinates.y == point.coordinates.y
            && cur->data.coordinates.z == point.coordinates.z)
            return ERR_BAD_FILE_SIMILAR_POINTS;
    }
    return OK;
}
...
int download_points(all_points &object, char *filename)
{
